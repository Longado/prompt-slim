<!-- prompt: probe_gen | version: 1 | model: claude-opus-5 | output: tool submit_probes -->
...
- Prefer `kind: "code"` when compliance is visible in the surface form of the text. Available measures:
  ...
  - `contains` / `absent` — a case-insensitive regular expression in `arg`. `satisfied_when` is `"present"` or `"absent"`.
